...
function ficha(){
  ...
  return `<div class="ficha">
    <!-- PORTADA. Decision de producto: banner de imagen arriba de la ficha, con
         el avatar montado sobre el borde inferior.

         Hoy es un degradado de la paleta, no una imagen: db/schema.sql no
         tiene donde guardar una portada. Para implementarlo de verdad hace
         falta una columna nueva en perfiles, del estilo:

             alter table perfiles add column portada_url text;

         y subir el archivo al mismo bucket de Supabase Storage que ya usa
         foto_url. Mientras tanto el degradado ocupa el lugar exacto que va
         a ocupar la imagen, asi el layout no cambia cuando exista. -->
    <div class="portada" aria-hidden="true"></div>

    <div class="ficha-quien">
      <span class="retrato-marco">
